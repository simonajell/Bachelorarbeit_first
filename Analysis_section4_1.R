...
ethCat1<-list()
ethCat1$ethW<-apply(logLOS_draws,1,function(x){sum(intval(x,dplyr::select(mutate(df,ethW=1,ethB=0,ethL=0,ethO=0),-"ethW")))/nrow(df)})
ethCat1$ethB<-apply(logLOS_draws,1,function(x){sum(intval(x,dplyr::select(mutate(df,ethW=0,ethB=1,ethL=0,ethO=0),-"ethW")))/nrow(df)})
ethCat1$ethL<-apply(logLOS_draws,1,function(x){sum(intval(x,dplyr::select(mutate(df,ethW=0,ethB=0,ethL=1,ethO=0),-"ethW")))/nrow(df)})
ethCat1$ethO<-apply(logLOS_draws,1,function(x){sum(intval(x,dplyr::select(mutate(df,ethW=0,ethB=0,ethL=0,ethO=1),-"ethW")))/nrow(df)})
...
p2<-ggplot(Effcat,aes(x=value,fill=assumption))+
  ...
  easy_remove_y_axis()+facet_grid(Ethnicity~.)+
