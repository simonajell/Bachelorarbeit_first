...
# Modell
set.seed(2022)
blogisticLOS_glm <- glm(as.numeric(LOS>median(LOS)) ~ Assignment + AGE + GENDER + Ethnicity,
                    family = "binomial", data = data)



predicts(blogisticLOS_glm, values = "all;all;all;all")
logLOS_draws_glm <- blogisticLOS_glm$effects

logLOS_draws <- blogisticLOS_glm %>%
  spread_draws(b_Intercept, b_AssignmentG ,
               b_AGE, b_GENDERM, b_EthnicityB, b_EthnicityL,
               b_EthnicityODU,
               ndraws = 1000) %>%
