...
catAII1<-function(draws,data){# this function computes \Delta_s under assumption (A.I’) for the clinical trial data of Tortora (2020)
  ...
  out<-list()
  out$ethB<-apply(draws,1,function(x){sum(intval(x,dplyr::select(mutate(df,ethW=0,ethB=1,ethL=0,ethO=0),-"ethW"))-intval(x,
        dplyr::select(mutate(df,ethW=1,ethB=0,ethL=0,ethO=0),-"ethW")))/nrow(df)})
  out$ethL<-apply(draws,1,function(x){sum(intval(x,dplyr::select(mutate(df,ethW=0,ethB=0,ethL=1,ethO=0),-"ethW"))-intval(x,
        dplyr::select(mutate(df,ethW=1,ethB=0,ethL=0,ethO=0),-"ethW")))/nrow(df)})
  out$ethO<-apply(draws,1,function(x){sum(intval(x,dplyr::select(mutate(df,ethW=0,ethB=0,ethL=0,ethO=1),-"ethW"))-intval(x,
        dplyr::select(mutate(df,ethW=1,ethB=0,ethL=0,ethO=0),-"ethW")))/nrow(df)})
  return(out)
}
catAII2<-function(draws,data){# this function computes \Delta_s under assumption (A.I’’) for the clinical trial data of Tortora (2020)
  ...
  df$assignmentG<-ifelse(df$assignmentG=="G",1,0)
  out<-list()
  out$ethB<-apply(draws,1,function(x){(sum(intval(x,dplyr::select(df[which(df$ethB==1),],-"ethW")))/length(which(df$ethB==1)))
    -(sum(intval(x,dplyr::select(df[which(df$ethW==1),],-"ethW")))/length(which(df$ethW==1)))})
  out$ethL<-apply(draws,1,function(x){(sum(intval(x,dplyr::select(df[which(df$ethL==1),],-"ethW")))/length(which(df$ethL==1)))
      -(sum(intval(x,dplyr::select(df[which(df$ethW==1),],-"ethW")))/length(which(df$ethW==1)))})
  out$ethO<-apply(draws,1,function(x){(sum(intval(x,dplyr::select(df[which(df$ethO==1),],-"ethW"))/length(which(df$ethO==1))))
        -(sum(intval(x,dplyr::select(df[which(df$ethW==1),],-"ethW"))/length(which(df$ethW==1))))})
  return(out)
}
################################################################################
#### From https://stackoverflow.com/questions/35511951/r-ggplot2-collapse-or-remove-segment-of-y-axis-from-scatter-plot :
squish_trans <- function(from, to, factor) { trans <- function(x) {
